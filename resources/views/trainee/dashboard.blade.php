@extends('layouts.trainee')

@section('title', 'Dashboard')

@section('content')
<div class="dashboard-header">
    <h1>Welcome, {{ Auth::guard('trainee')->user()->full_name }}</h1>
    <p>Track your learning progress and achievements</p>
</div>

<!-- Statistics Cards -->
<div class="stats-grid">
    <div class="stat-card">
        <div class="stat-icon" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);">
            <i class="fas fa-book"></i>
        </div>
        <div class="stat-info">
            <div class="stat-number">{{ $stats['enrolled_courses'] }}</div>
            <div class="stat-label">Available Courses</div>
        </div>
    </div>

    <div class="stat-card">
        <div class="stat-icon" style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%);">
            <i class="fas fa-check-circle"></i>
        </div>
        <div class="stat-info">
            <div class="stat-number">{{ $stats['completed_courses'] }}</div>
            <div class="stat-label">Completed Courses</div>
        </div>
    </div>

    <div class="stat-card">
        <div class="stat-icon" style="background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%);">
            <i class="fas fa-clipboard-check"></i>
        </div>
        <div class="stat-info">
            <div class="stat-number">{{ $stats['total_assessments'] }}</div>
            <div class="stat-label">Assessments Taken</div>
        </div>
    </div>

    <div class="stat-card">
        <div class="stat-icon" style="background: linear-gradient(135deg, #43e97b 0%, #38f9d7 100%);">
            <i class="fas fa-certificate"></i>
        </div>
        <div class="stat-info">
            <div class="stat-number">{{ $stats['certificates']->count() }}</div>
            <div class="stat-label">Certificates Earned</div>
        </div>
    </div>
</div>

<!-- Available Courses -->
<div class="section">
    <div class="section-header">
        <h2><i class="fas fa-book-open"></i> Available Courses</h2>
        <a href="{{ route('trainee.courses.index') }}" class="btn-link">View All <i class="fas fa-arrow-right"></i></a>
    </div>
    
    @if($availableCourses->count() > 0)
    <div class="courses-grid">
        @foreach($availableCourses->take(6) as $course)
        <div class="course-card">
            <div class="course-header">
                <h3>{{ $course->title }}</h3>
                <span class="course-code">{{ $course->code }}</span>
            </div>
            <p class="course-description">{{ \Illuminate\Support\Str::limit($course->description, 100) }}</p>
            <div class="course-meta">
                <span><i class="fas fa-clock"></i> {{ $course->duration_hours }} Hours</span>
                <span><i class="fas fa-question-circle"></i> {{ $course->questionPools->count() }} Questions</span>
            </div>
            <div class="course-status">
                @if($course->has_passed)
                    <span class="badge badge-success"><i class="fas fa-check"></i> Passed</span>
                @elseif($course->has_taken)
                    <span class="badge badge-warning"><i class="fas fa-redo"></i> Retake Available</span>
                @else
                    <span class="badge badge-info"><i class="fas fa-play"></i> Available</span>
                @endif
            </div>
            <div class="course-actions">
                <a href="{{ route('trainee.courses.show', $course->id) }}" class="btn btn-primary btn-sm">
                    @if($course->has_passed)
                        <i class="fas fa-eye"></i> View Result
                    @elseif($course->has_taken)
                        <i class="fas fa-redo"></i> Retake
                    @else
                        <i class="fas fa-play"></i> Start Course
                    @endif
                </a>
            </div>
        </div>
        @endforeach
    </div>
    @else
    <div class="empty-state">
        <i class="fas fa-book-open"></i>
        <p>No courses available at the moment.</p>
    </div>
    @endif
</div>

<!-- Recent Results -->
@if($recentResults->count() > 0)
<div class="section">
    <div class="section-header">
        <h2><i class="fas fa-chart-line"></i> Recent Assessment Results</h2>
    </div>
    <div class="results-list">
        @foreach($recentResults as $result)
        <div class="result-item">
            <div class="result-course">
                <h4>{{ $result->course->title }}</h4>
                <span class="result-date">{{ $result->completed_at->format('M d, Y') }}</span>
            </div>
            <div class="result-score">
                <div class="score-circle {{ $result->status === 'passed' ? 'passed' : 'failed' }}">
                    <span>{{ number_format($result->percentage, 0) }}%</span>
                </div>
            </div>
            <div class="result-status">
                @if($result->status === 'passed')
                    <span class="badge badge-success"><i class="fas fa-check"></i> Passed</span>
                @else
                    <span class="badge badge-danger"><i class="fas fa-times"></i> Failed</span>
                @endif
            </div>
            <div class="result-actions">
                <a href="{{ route('trainee.assessment.result', $result->id) }}" class="btn btn-sm btn-outline">View Details</a>
                @if($result->status === 'passed')
                    <a href="{{ route('trainee.certificates.view', $result->id) }}" class="btn btn-sm btn-primary">
                        <i class="fas fa-certificate"></i> Certificate
                    </a>
                @endif
            </div>
        </div>
        @endforeach
    </div>
</div>
@endif
@endsection

